...
def extract_wait_time_and_oxygen(comment):
    ...
    pattern = r"Wait time:\s*(\d+)\s*minutes(?:.*?\b(\d+)\s*\
        (?:liter|liters|LPM|L|lts|LITERS|lt|l)\b)?"
...
# TODO: we can safely delete this function.
# implementation above removes CSV handling
'''
...
# TODO: we can safely delete this function.
# replaced with df = dt.standardize_address(dt.standardize_name(df) in main.py
'''
